-- https://github.com/Turtle-Brand/Turtle-Spy/blob/main/source.lua
...
local function toUnicode(string)
	...
	for _i, v in utf8.codes(string) do codepoints = codepoints .. v .. ', ' end
...
CloseInfoFrame2.MouseButton1Click:Connect(
	function() BrowserHeader.Visible = not BrowserHeader.Visible end)
...
RemoteBrowserFrame.VerticalScrollBarPosition =
	Enum.VerticalScrollBarPosition.Left
RemoteBrowserFrame.ScrollBarImageColor3 =
	colorSettings['Main']['ScrollBarImageColor']

RemoteButton2.Name = 'RemoteButton'
RemoteButton2.Parent = RemoteBrowserFrame
RemoteButton2.BackgroundColor3 =
	colorSettings['RemoteButtons']['BackgroundColor']
	...
	function()
		...
		for i, v in pairs(game:GetDescendants()) do
			if isA(v, 'RemoteEvent') or isA(v, 'RemoteFunction') then
				...
				local connection = bButton.MouseButton1Click:Connect(
					function() setclipboard(GetFullPathOfAnInstance(v) .. fireFunction) end)
...
RemoteScrollFrame.VerticalScrollBarPosition =
	Enum.VerticalScrollBarPosition.Left
RemoteScrollFrame.ScrollBarImageColor3 =
	colorSettings['Main']['ScrollBarImageColor']

RemoteButton.Name = 'RemoteButton'
RemoteButton.Parent = RemoteScrollFrame
RemoteButton.BackgroundColor3 =
	colorSettings['RemoteButtons']['BackgroundColor']
...
InfoButtonsScroll.BackgroundColor3 =
	colorSettings['Main']['MainBackgroundColor']
...
InfoButtonsScroll.VerticalScrollBarPosition =
	Enum.VerticalScrollBarPosition.Left
InfoButtonsScroll.ScrollBarImageColor3 =
	colorSettings['Main']['ScrollBarImageColor']
...
CopyScriptPath.BackgroundColor3 =
	colorSettings['MainButtons']['BackgroundColor']
...
CopyDecompiled.BackgroundColor3 =
	colorSettings['MainButtons']['BackgroundColor']
...
CloseInfoFrame.MouseButton1Click:Connect(
	function()
		InfoFrame.Visible = false
		InfoFrameOpen = false
		mainFrame.Size = UDim2.new(0, 207, 0, 35)
...
OpenInfoFrame.MouseButton1Click:Connect(
	function()
		...
		for b, v in pairs(remotes) do
			if v == remote then
				...
				for i2, v2 in pairs(remoteArgs) do
					if table.unpack(remoteArgs[b]) == table.unpack(args) then i = b end
				end
			end
		end
	else
...
CopyCode.MouseButton1Click:Connect(
	function()
...
RunCode.MouseButton1Click:Connect(
	function()
...
CopyScriptPath.MouseButton1Click:Connect(
	function()
		-- get remote index
...
local decompiling
CopyDecompiled.MouseButton1Click:Connect(
	function()
		local remote = FindRemote(lookingAt, lookingAtArgs)
		...
		if not decompiling and remote and lookingAt then
			...
			local success = {
				pcall(function() setclipboard(decompile(remoteScripts[remote])) end),
			}
			decompiling = false
			if success[1] then
...
BlockRemote.MouseButton1Click:Connect(
	function()
		...
		if lookingAt and not bRemote then
			...
			if remote then
				remoteButtons[remote].Parent.RemoteName.TextColor3 = Color3.fromRGB(
					225, 177, 44)
			end
...
IgnoreRemote.MouseButton1Click:Connect(
	function()
...
WhileLoop.MouseButton1Click:Connect(
	function()
		if not lookingAt then return end
...
DoNotStack.MouseButton1Click:Connect(
	function()
		if lookingAt then
...
CopyReturn.MouseButton1Click:Connect(
	function()
		local remote = FindRemote(lookingAt, lookingAtArgs)
...
RemoteScrollFrame.ChildAdded:Connect(
	function(child)
		...
		local connection = child.MouseButton1Click:Connect(
			function()
				...
				OpenInfoFrame.Text = '>'
				InfoFrame.Visible = true
				Code.Text = GetFullPathOfAnInstance(remote) .. fireFunction ..
					            convertTableToString(args) .. ')'
				-- gets text size and updates code box's size accordingly
				local textsize = TextService:GetTextSize(
					Code.Text, Code.TextSize, Code.Font, Vector2.new(math.huge, math.huge))
...
function addToList(event, remote, ...)
	...
	if not i then
		...
		remoteScripts[#remotes] = (isSynapse() and getcallingscript() or
			                          rawget(getfenv(0), 'script'))

		-- clone a little baby of the remotebutton
		rButton.Parent = RemoteScrollFrame
		rButton.Visible = true
		local numberTextsize = getTextSize(
			TextService, rButton.Number.Text, rButton.Number.TextSize,
				rButton.Number.Font, Vector2.new(math.huge, math.huge))
		...
		local numberTextsize = getTextSize(
			TextService, remoteButtons[i].Text, remoteButtons[i].TextSize,
				remoteButtons[i].Font, Vector2.new(math.huge, math.huge))
		remoteButtons[i].Parent.RemoteName.Position = UDim2.new(
			0, numberTextsize.X + 10, 0, 0)
		remoteButtons[i].Parent.RemoteName.Size = UDim2.new(
			0, 149 - numberTextsize.X, 0, 26)

		-- update the arguments
		remoteArgs[i] = args

		-- update the codebox if the player is looking at it
		if lookingAt and lookingAt == remote and lookingAtButton == remoteButtons[i] and
			InfoFrame.Visible then
			local fireFunction = ':FireServer('
			if isA(remote, 'RemoteFunction') then fireFunction = ':InvokeServer(' end
			Code.Text = GetFullPathOfAnInstance(remote) .. fireFunction ..
				            convertTableToString(remoteArgs[i]) .. ')'
			local textsize = getTextSize(
				TextService, Code.Text, Code.TextSize, Code.Font,
...
local OldEvent
OldEvent = hookfunction(
	Instance.new('RemoteEvent').FireServer, function(Self, ...)
...
local OldFunction
OldFunction = hookfunction(
	Instance.new('RemoteFunction').InvokeServer, function(Self, ...)
		if not checkcaller() and table.find(BlockList, Self) then
			return
		elseif table.find(IgnoreList, Self) then
			-- if ignored then don't call the addToList
